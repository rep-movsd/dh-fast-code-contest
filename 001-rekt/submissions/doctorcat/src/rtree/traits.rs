...
use rtree;
use rtree::num::{Signed, BigInt, BigRational, zero};
use rtree::boundingvolume::BoundingRect;
use rtree::point_traits::{PointN, PointNExtensions, TwoDimensional};
use std::fmt::Debug;
...
impl <S>  SpatialObject for S where S: HasPosition {
    ...
    #[inline]
    fn rank(&self) -> u32 {
        rtree::point_traits::PointN::rank(&self.position())
    }
}
